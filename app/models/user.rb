...
class User < ActiveRecord::Base
  ...
  has_many :active_projects,   :through => :project_users, :source => :project, :conditions => 'projects.completed_on IS NULL',     :order => 'projects.name ASC'
  ...
  def avatar_url
    if !avatar?
      "http://gravatar.com/avatar/#{Digest::MD5.hexdigest email}?s=50&d=" + URI.encode("#{AppConfig.site_url}/themes/#{AppConfig.site_theme}/images/avatar.png")
    else
      avatar.url(:thumb)
    end
  end
