...
class BasecampController < ApplicationController
  ...
  def msg_comment
	...
	unless @comment.can_be_seen_by(@logged_user)
      render :text => 'Error', :status => 404
      return
    end
  end
  ...
  def msg_create_comment
	...
	unless message.comment_can_be_added_by(@logged_user)
      render :text => 'Error', :status => 403
      return
    end
  ...
  def projects_msg_create
    ...
    if @message.save
      ...
      unless @request_fields[:notify].nil?
        valid_user_ids = @request_fields[:notify].collect do |user_id|
        ...
        User.all(:conditions => { :id => valid_user_ids }).each do |user|
          @message.send_notification(user)
        end
      end

      # Handle file attachments
      unless @request_fields[:attachments].nil?
        # TODO
      end
  ...
  # /msg/get/#{message_ids}
  def msg_get
  	message_ids = params[:ids].split(',').collect{ |id| id.to_i }.uniq

  	@messages = ProjectMessage.all(:conditions => { :id => message_ids }).reject do |message|
  ...
  def projects_msg_archive
  	...
  	@messages = ProjectMessage.all(:conditions => msg_conditions)
  end
  ...
  def todos_create_item
    ...
    unless ProjectTaskList.can_be_created_by(@logged_user, task_list)
      render :text => 'Error', :status => 403
      return
    end
  ...
  def milestones_uncomplete
    ...
    unless @milestone.completed_by.nil?
  ...
  def time_save_entry
    ...
    unless ProjectTime.can_be_created_by(@logged_user, project)
      render :text => 'Error', :status => 403
      return
    end
  ...
  def time_report
    ...
    # Date range
  	start_time = params[:from] == '0' ? (Time.now.utc - 90.days) : Time.parse(params[:from])
      ...
      ["#{user_filter} done_date >= ? AND done_date <= ? AND is_private = ?", start_time, end_time, false]

  	# Now we can grab the times!
  	filter = params[:filter]
  	unless filter.nil?
      filter_prefix = filter[0..0]
      filter_id     = filter[1..-1]
      ...
      if filter_prefix == 'c'
        ...
        project_ids = company.projects.collect { |project| project.id }.join(',')

        time_conditions[0] += " AND project_id IN (#{project_ids})"
        @times = ProjectTime.all(:conditions => time_conditions)
      elsif filter_prefix == 'p'
        # Filter by project
        begin
